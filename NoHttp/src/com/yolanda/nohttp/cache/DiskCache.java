...
/**
 * </br>
 * Created in Dec 14, 2015 5:53:26 PM
 * 
 * @author YOLANDA;
 */
public class DiskCache implements Cache {
	...
	public synchronized Entrance get(String key) {
		...
		try {
			...
			Logger.d("%s: %s", file.getAbsolutePath(), e.toString());
			remove(key);
			return null;
		} finally {
			if (cis != null) {
	...
	public synchronized void put(String key, Entrance entry) {
		...
		if (!deleted) {
			Logger.d("Could not clean up file %s", file.getAbsolutePath());
		}
	}
	...
	public synchronized void remove(String key) {
		...
		removeEntry(key);
		if (!deleted) {
			Logger.d("Could not delete cache entry for key=%s, filename=%s", key, getFilenameForKey(key));
		}
	}
	...
	private void pruneIfNeeded(int neededSpace) {
		...
		Logger.v("Pruning old cache entries.");
		...
		while (iterator.hasNext()) {
			...
			if (deleted) {
				...
				Logger.d("Could not delete cache entry for key=%s, filename=%s", e.key, getFilenameForKey(e.key));
			}
			iterator.remove();
			prunedFiles++;
		...
		Logger.v("pruned %d files, %d bytes, %d ms", prunedFiles, (mTotalSize - before), SystemClock.elapsedRealtime() - startTime);
	}
